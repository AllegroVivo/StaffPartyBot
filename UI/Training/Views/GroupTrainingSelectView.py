...
__all__ = ("GroupTrainingSelectView",)

################################################################################
class GroupTrainingSelectView(FroggeView):

    def __init__(self, user: User, options: List[SelectOption]):
        
        super().__init__(user, close_on_complete=True)
        
        self.add_item(GroupTrainingSelect(options))
        self.add_item(CloseMessageButton())
        
################################################################################
class GroupTrainingSelect(Select):
    ...
    def __init__(self, options: List[SelectOption]):
        ...
        super().__init__(
            placeholder="Select a group training...",
            options=options,
            min_values=1,
            max_values=1,
